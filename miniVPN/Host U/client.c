...
int setupTCPClient(const char* hostname, int port)
{
   ...
   // server_addr.sin_addr.s_addr = inet_addr ("10.0.2.5"); 
...
void send_credentials(SSL *ssl, char *user, char *passwd) {
   ...
   // sending the packet length and then the packet itself
   tls->tlsh_len = htons(cred->pckt_len);
   err = SSL_write(ssl, tls, sizeof(struct tls_header));
   CHK_SSL(err);
   err = SSL_write(ssl, cred, cred->pckt_len);
   CHK_SSL(err);
}
...
int reading_result(SSL *ssl) {
   ...
   memset(buffer, 0, 4);
   struct tls_header *tls = (struct tls_header *) buffer;
   // reading packet length and then the packet itself
...
void setup_login(int sockfd, SSL *ssl, char *user, char *password) {
   ...
   if (res == 1) {
      printf("Welcome! You are logged in the VPN.\n");
   } else {
